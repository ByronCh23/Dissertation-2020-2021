# -*- coding: utf-8 -*-
"""
Created on Mon Aug  9 21:16:42 2021
...
X_BM = Merge[["MRP","SMB","HML","Momentum"]]
Z_BM = sm.add_constant(X_BM)
Z_BM.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_BM = RESULTS_BM.params[4]

#Find the estimated FF returns
CAR_RETURNS_BM = ALPHA_BM + BETA1_BM*Merge["MRP"] + BETA2_BM*Merge["SMB"] + BETA3_BM*Merge["HML"] + BETA4_BM*Merge["Momentum"]
...
X_RED_BM = REDUCED_BM[["MRP","SMB","HML","Momentum"]]
Z_RED_BM = sm.add_constant(X_RED_BM)
Z_RED_BM.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_BM = RESULTS_RED_BM.params.beta4

#Create covid database 
COVID_BM = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the FF returns for the Covid database
CAR_RETURNS_COVID_BM = ALPHA_RED_BM + BETA1_RED_BM*(COVID_BM["MRP"]) + BETA2_RED_BM*(COVID_BM["SMB"]) + BETA3_RED_BM*(COVID_BM["HML"]) + BETA4_RED_BM*(COVID_BM["Momentum"])

#Acquire the Actual FF returns for covid era that were estimated previously
CAR_RETURNS_RED_BM = CAR_RETURNS_BM[CAR_RETURNS_BM.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_BM = CAR_RETURNS_RED_BM - CAR_RETURNS_COVID_BM
Abnormal_CAR_sum_BM = sum(ABNORMAL_CAR_BM)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_CS = Merge[["MRP","SMB","HML","Momentum"]]
Z_CS = sm.add_constant(X_CS)
Z_CS.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_CS = RESULTS_CS.params[4]

#Find the estimated CAR returns
CAR_RETURNS_CS = ALPHA_CS + BETA1_CS*Merge["MRP"] + BETA2_CS*Merge["SMB"] + BETA3_CS*Merge["HML"] + BETA4_CS*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_CS = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_CS = REDUCED_CS["ER"]
X_RED_CS = REDUCED_CS[["MRP","SMB","HML","Momentum"]]
Z_RED_CS = sm.add_constant(X_RED_CS)
Z_RED_CS.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_CS = RESULTS_RED_CS.params.beta4

#Create covid database 
COVID_CS = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_CS = ALPHA_RED_CS + BETA1_RED_CS*(COVID_CS["MRP"]) + BETA2_RED_CS*(COVID_CS["SMB"]) + BETA3_RED_CS*(COVID_CS["HML"]) + BETA4_RED_CS*(COVID_CS["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_CS = CAR_RETURNS_CS[CAR_RETURNS_CS.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_CS = CAR_RETURNS_RED_CS - CAR_RETURNS_COVID_CS
Abnormal_CAR_sum_CS = sum(ABNORMAL_CAR_CS)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_CC = Merge[["MRP","SMB","HML","Momentum"]]
Z_CC = sm.add_constant(X_CC)
Z_CC.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_CC = RESULTS_CC.params[4]

#Find the estimated CAR returns
CAR_RETURNS_CC = ALPHA_CC + BETA1_CC*Merge["MRP"] + BETA2_CC*Merge["SMB"] + BETA3_CC*Merge["HML"] + BETA4_CC*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_CC = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_CC = REDUCED_CC["ER"]
X_RED_CC = REDUCED_CC[["MRP","SMB","HML","Momentum"]]
Z_RED_CC = sm.add_constant(X_RED_CC)
Z_RED_CC.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_CC = RESULTS_RED_CC.params.beta4

#Create covid database 
COVID_CC = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_CC = ALPHA_RED_CC + BETA1_RED_CC*(COVID_CC["MRP"]) + BETA2_RED_CC*(COVID_CC["SMB"]) + BETA3_RED_CC*(COVID_CC["HML"]) + BETA4_RED_CC*(COVID_CC["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_CC = CAR_RETURNS_CC[CAR_RETURNS_CC.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_CC = CAR_RETURNS_RED_CC - CAR_RETURNS_COVID_CC
Abnormal_CAR_sum_CC = sum(ABNORMAL_CAR_CC)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_CD = Merge[["MRP","SMB","HML","Momentum"]]
Z_CD = sm.add_constant(X_CD)
Z_CD.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_CD = RESULTS_CD.params[4]

#Find the estimated CAR returns
CAR_RETURNS_CD = ALPHA_CD + BETA1_CD*Merge["MRP"] + BETA2_CD*Merge["SMB"] + BETA3_CD*Merge["HML"] + BETA4_CD*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_CD = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_CD = REDUCED_CD["ER"]
X_RED_CD = REDUCED_CD[["MRP","SMB","HML","Momentum"]]
Z_RED_CD = sm.add_constant(X_RED_CD)
Z_RED_CD.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_CD = RESULTS_RED_CD.params.beta4

#Create covid database 
COVID_CD = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_CD = ALPHA_RED_CD + BETA1_RED_CD*(COVID_CD["MRP"]) + BETA2_RED_CD*(COVID_CD["SMB"]) + BETA3_RED_CD*(COVID_CD["HML"]) + BETA4_RED_CD*(COVID_CD["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_CD = CAR_RETURNS_CD[CAR_RETURNS_CD.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_CD = CAR_RETURNS_RED_CD - CAR_RETURNS_COVID_CD
Abnormal_CAR_sum_CD = sum(ABNORMAL_CAR_CD)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_E = Merge[["MRP","SMB","HML","Momentum"]]
Z_E = sm.add_constant(X_E)
Z_E.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_E = RESULTS_E.params[4]

#Find the estimated CAR returns
CAR_RETURNS_E = ALPHA_E + BETA1_E*Merge["MRP"] + BETA2_E*Merge["SMB"] + BETA3_E*Merge["HML"] + BETA4_E*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_E = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_E = REDUCED_E["ER"]
X_RED_E = REDUCED_E[["MRP","SMB","HML","Momentum"]]
Z_RED_E = sm.add_constant(X_RED_E)
Z_RED_E.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_E = RESULTS_RED_E.params.beta4

#Create covid database 
COVID_E = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_E = ALPHA_RED_E + BETA1_RED_E*(COVID_E["MRP"]) + BETA2_RED_E*(COVID_E["SMB"]) + BETA3_RED_E*(COVID_E["HML"]) + BETA4_RED_E*(COVID_E["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_E = CAR_RETURNS_E[CAR_RETURNS_E.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_E = CAR_RETURNS_RED_E - CAR_RETURNS_COVID_E
Abnormal_CAR_sum_E = sum(ABNORMAL_CAR_E)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_FS = Merge[["MRP","SMB","HML","Momentum"]]
Z_FS = sm.add_constant(X_FS)
Z_FS.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_FS = RESULTS_FS.params[4]

#Find the estimated CAR returns
CAR_RETURNS_FS = ALPHA_FS + BETA1_FS*Merge["MRP"] + BETA2_FS*Merge["SMB"] + BETA3_FS*Merge["HML"] + BETA4_FS*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_FS = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_FS = REDUCED_FS["ER"]
X_RED_FS = REDUCED_FS[["MRP","SMB","HML","Momentum"]]
Z_RED_FS = sm.add_constant(X_RED_FS)
Z_RED_FS.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_FS = RESULTS_RED_FS.params.beta4

#Create covid database 
COVID_FS = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_FS = ALPHA_RED_FS + BETA1_RED_FS*(COVID_FS["MRP"]) + BETA2_RED_FS*(COVID_FS["SMB"]) + BETA3_RED_FS*(COVID_FS["HML"]) + BETA4_RED_FS*(COVID_FS["Momentum"])


#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_FS = CAR_RETURNS_FS[CAR_RETURNS_FS.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_FS = CAR_RETURNS_RED_FS - CAR_RETURNS_COVID_FS
Abnormal_CAR_sum_FS = sum(ABNORMAL_CAR_FS)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_H = Merge[["MRP","SMB","HML","Momentum"]]
Z_H = sm.add_constant(X_H)
Z_H.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_H = RESULTS_H.params[4]

#Find the estimated CAR returns
CAR_RETURNS_H = ALPHA_H + BETA1_H*Merge["MRP"] + BETA2_H*Merge["SMB"] + BETA3_H*Merge["HML"] + BETA4_H*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_H = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_H = REDUCED_H["ER"]
X_RED_H = REDUCED_H[["MRP","SMB","HML","Momentum"]]
Z_RED_H = sm.add_constant(X_RED_H)
Z_RED_H.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_H = RESULTS_RED_H.params.beta4

#Create covid database 
COVID_H = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_H = ALPHA_RED_H + BETA1_RED_H*(COVID_H["MRP"]) + BETA2_RED_H*(COVID_H["SMB"]) + BETA3_RED_H*(COVID_H["HML"]) + BETA4_RED_H*(COVID_H["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_H = CAR_RETURNS_H[CAR_RETURNS_H.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_H = CAR_RETURNS_RED_H - CAR_RETURNS_COVID_H
Abnormal_CAR_sum_H = sum(ABNORMAL_CAR_H)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_I = Merge[["MRP","SMB","HML","Momentum"]]
Z_I = sm.add_constant(X_I)
Z_I.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_I = RESULTS_I.params[4]

#Find the estimated CAR returns
CAR_RETURNS_I = ALPHA_I + BETA1_I*Merge["MRP"] + BETA2_I*Merge["SMB"] + BETA3_I*Merge["HML"] + BETA4_I*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_I = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_I = REDUCED_I["ER"]
X_RED_I = REDUCED_I[["MRP","SMB","HML","Momentum"]]
Z_RED_I = sm.add_constant(X_RED_I)
Z_RED_I.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_I = RESULTS_RED_I.params.beta4

#Create covid database 
COVID_I = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_I = ALPHA_RED_I + BETA1_RED_I*(COVID_I["MRP"]) + BETA2_RED_I*(COVID_I["SMB"]) + BETA3_RED_I*(COVID_I["HML"]) + BETA4_RED_I*(COVID_I["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_I = CAR_RETURNS_I[CAR_RETURNS_I.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_I = CAR_RETURNS_RED_I - CAR_RETURNS_COVID_I
Abnormal_CAR_sum_I = sum(ABNORMAL_CAR_I)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_RE = Merge[["MRP","SMB","HML","Momentum"]]
Z_RE = sm.add_constant(X_RE)
Z_RE.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RE = RESULTS_RE.params[4]

#Find the estimated CAR returns
CAR_RETURNS_RE = ALPHA_RE + BETA1_RE*Merge["MRP"] + BETA2_RE*Merge["SMB"] + BETA3_RE*Merge["HML"] + BETA4_RE*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_RE = Merge.loc[Merge.index < "31-12-2019"]
Y_RED_RE = REDUCED_RE["ER"]
X_RED_RE = REDUCED_RE[["MRP","SMB","HML","Momentum"]]
Z_RED_RE = sm.add_constant(X_RED_RE)
Z_RED_RE.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_RED_RE = RESULTS_RED_RE.params.beta4

#Create covid database 
COVID_RE = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_RETURNS_COVID_RE = ALPHA_RED_RE + BETA1_RED_RE*(COVID_RE["MRP"]) + BETA2_RED_RE*(COVID_RE["SMB"]) + BETA3_RED_RE*(COVID_RE["HML"]) + BETA4_RED_RE*(COVID_RE["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_RETURNS_RED_RE = CAR_RETURNS_RE[CAR_RETURNS_RE.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_RE = CAR_RETURNS_RED_RE - CAR_RETURNS_COVID_RE
Abnormal_CAR_sum_RE = sum(ABNORMAL_CAR_RE)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_T = Merge[["MRP","SMB","HML","Momentum"]]
Z_T = sm.add_constant(X_T)
Z_T.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_T = RESULTS_T.params[4]

#Find the estimated CAR returns
CAR_TTURNS_T = ALPHA_T + BETA1_T*Merge["MRP"] + BETA2_T*Merge["SMB"] + BETA3_T*Merge["HML"] + BETA4_T*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_T = Merge.loc[Merge.index < "31-12-2019"]
Y_TD_T = REDUCED_T["ER"]
X_TD_T = REDUCED_T[["MRP","SMB","HML","Momentum"]]
Z_TD_T = sm.add_constant(X_TD_T)
Z_TD_T.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_TD_T = RESULTS_TD_T.params.beta4

#Create covid database 
COVID_T = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_TTURNS_COVID_T = ALPHA_TD_T + BETA1_TD_T*(COVID_T["MRP"]) + BETA2_TD_T*(COVID_T["SMB"]) + BETA3_TD_T*(COVID_T["HML"]) + BETA4_TD_T*(COVID_T["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_TTURNS_TD_T = CAR_TTURNS_T[CAR_TTURNS_T.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_T = CAR_TTURNS_TD_T - CAR_TTURNS_COVID_T
Abnormal_CAR_sum_T = sum(ABNORMAL_CAR_T)
...
Merge["MRP"] =  Merge["S&P500"] -  Merge["RF"]
X_U = Merge[["MRP","SMB","HML","Momentum"]]
Z_U = sm.add_constant(X_U)
Z_U.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_U = RESULTS_U.params[4]

#Find the estimated CAR returns
CAR_UTURNS_U = ALPHA_U + BETA1_U*Merge["MRP"] + BETA2_U*Merge["SMB"] + BETA3_U*Merge["HML"] + BETA4_U*Merge["Momentum"]

#EVENT STUDY Code
#Create pre-covid database and run OLS regression as before with it
REDUCED_U = Merge.loc[Merge.index < "31-12-2019"]
Y_UD_U = REDUCED_U["ER"]
X_UD_U = REDUCED_U[["MRP","SMB","HML","Momentum"]]
Z_UD_U = sm.add_constant(X_UD_U)
Z_UD_U.columns = ["alpha","beta1","beta2","beta3","beta4"]
...
BETA4_UD_U = RESULTS_UD_U.params.beta4

#Create covid database 
COVID_U = Merge.loc[Merge.index > "31-12-2019"]

#Estimate the CAR returns for the Covid database
CAR_UTURNS_COVID_U = ALPHA_UD_U + BETA1_UD_U*(COVID_U["MRP"]) + BETA2_UD_U*(COVID_U["SMB"]) + BETA3_UD_U*(COVID_U["HML"]) + BETA4_UD_U*(COVID_U["Momentum"])

#Acquire the Actual CAR returns for covid era that were estimated previously
CAR_UTURNS_UD_U = CAR_UTURNS_U[CAR_UTURNS_U.index > "31-12-2019"]

#Estimate Abnormal returns
ABNORMAL_CAR_U = CAR_UTURNS_UD_U - CAR_UTURNS_COVID_U
Abnormal_CAR_sum_U = sum(ABNORMAL_CAR_U)

#Create Dataframe containing all the abnormal returns
Dict = [ABNORMAL_CAR_BM,ABNORMAL_CAR_CS,ABNORMAL_CAR_CC,ABNORMAL_CAR_CD,ABNORMAL_CAR_E,ABNORMAL_CAR_FS,ABNORMAL_CAR_H,ABNORMAL_CAR_I,ABNORMAL_CAR_RE,ABNORMAL_CAR_T,ABNORMAL_CAR_U]
Di = pd.concat(Dict, axis=1)
x ={'Abnormal Returns' : [Abnormal_CAR_sum_BM,Abnormal_CAR_sum_CS,Abnormal_CAR_sum_CC,Abnormal_CAR_sum_CD,Abnormal_CAR_sum_E,Abnormal_CAR_sum_FS,Abnormal_CAR_sum_H,Abnormal_CAR_sum_I,Abnormal_CAR_sum_RE,Abnormal_CAR_sum_T,Abnormal_CAR_sum_U], 'Sector' : ['Basic Materials','Communication Services','Consumer Cyclical','Consumer Defensive','Energy','Financial Services','Healthcare','Industrials','Real Estate','Technology','Utilities']}
...
BM, = plt.plot(ABNORMAL_CAR_BM)
CS, = plt.plot(ABNORMAL_CAR_CS)
CC, = plt.plot(ABNORMAL_CAR_CC)
CD, = plt.plot(ABNORMAL_CAR_CD)
EN, = plt.plot(ABNORMAL_CAR_E)
FS, = plt.plot(ABNORMAL_CAR_FS)
HE, = plt.plot(ABNORMAL_CAR_H)
IN, = plt.plot(ABNORMAL_CAR_I)
RE, = plt.plot(ABNORMAL_CAR_RE)
TE, = plt.plot(ABNORMAL_CAR_T)
UT, = plt.plot(ABNORMAL_CAR_U)
